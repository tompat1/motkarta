...
const previewHeaders = { 'cache-control': 'no-store', 'x-motkarta-preview': 'ai-synthesis-readonly-v1' };
...
export default {
  async fetch(request: Request, env: PreviewEnv): Promise<Response> {
    ...
    if (pathname === '/api/concierge') {
      ...
      // Preview enables only grounded Gemma synthesis. Hybrid retrieval remains off
      // until an independently reviewed holdout establishes a production threshold.
    ...
    result.headers.set('x-motkarta-preview', 'ai-synthesis-readonly-v1');
