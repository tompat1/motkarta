...
export function parseD1Export(value) {
  ...
  return { places: rowsToPlaceInputs(rows, evidence, tags), identities };
}
...
export async function auditCatalog(publicPlaces, snapshot) {
  ...
  if (corroboratedSameIds.length !== eligibleD1) blockers.push('public_map_ids_do_not_cover_d1');
  if (sameIds.filter((row) => row.eligible && row.hashMatches).length !== eligibleD1) blockers.push('public_vectors_do_not_match_d1');
  ...
  return {
    auditVersion: 'concierge-catalog-audit-v1', corpusVersion: VERSIONS.corpus, productionReady: false,
...
async function main() {
  const { values } = parseArgs({ options: { 'write-sql': { type: 'string' }, 'd1-export': { type: 'string' }, public: { type: 'string', default: 'public/data/places.json' }, output: { type: 'string', default: '.tmp/concierge-readiness' } } });
  ...
  const report = await auditCatalog(publicPlaces, snapshot);
  ...
  await writeFile(resolve(values.output, 'canonical-d1.json'), JSON.stringify({ source: 'd1', auditInputs: report.inputs, places: snapshot.places }, null, 2) + '\n');
